  ...
  const handleRemove = async () => {
    ...
    try {
        ...
        const updatedPosts = posts.filter(
            (post) => post.postDoc._delegate._key.path.segments.at(-1) !== id
        );
  ...
  useEffect(() => {
    if (userDoc.document != null) {
      ...
      postsAdded.forEach((post) => {
        ...
        if (
          post["postDoc"]["_delegate"]["_key"]["path"]["segments"].at(-1) ==
            newPost["postDoc"]["_delegate"]["_key"]["path"]["segments"].at(
              -1
            ) &&
      ...
      {document && (
        <>
          ...
          <Link to={`/profile/${document.creator}`}>
            Creator: {document.creatorName}
          </Link>
          <p>
            Created At:{" "}
            {document.createdAt && document.createdAt.toDate().toString()}
          </p>
          <p>
            Updated At:{" "}
            {document.updatedAt && document.updatedAt.toDate().toString()}
          </p>
        </>
      )}
      {!editable && !disabled && (
